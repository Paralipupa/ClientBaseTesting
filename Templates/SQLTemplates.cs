...
namespace ClientBaseTesting.Templates
{
    class SQLTemplates
    {
        ...
        public static string SELECT_NOMENCLATURE_COST = "SELECT * FROM Цена WHERE КодПрайслистНоменклатура={0} AND КодТипЦены={1} ORDER BY КодСкидка, Копия, Сумма";
        public static string SELECT_PRICELIST_GRUPPA = "SELECT DISTINCT `Прайслист-Номенклатура`.КодПрайслист, ГруппаПрайслистНоменклатура.КодГруппаНоменклатура, ГруппаНоменклатура.Наименование FROM `Прайслист-Номенклатура` INNER JOIN (ГруппаНоменклатура INNER JOIN ГруппаПрайслистНоменклатура ON ГруппаНоменклатура.Код = ГруппаПрайслистНоменклатура.КодГруппаНоменклатура) ON `Прайслист-Номенклатура`.Код = ГруппаПрайслистНоменклатура.КодПрайслистНоменклатура WHERE(((`Прайслист-Номенклатура`.КодПрайслист)={0}))";
    }
}
